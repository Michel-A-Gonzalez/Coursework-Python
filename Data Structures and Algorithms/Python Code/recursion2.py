...
#  Description: This program holds mutliple functions to help understand
#               and practice with recursive problems
...
def splitArray(nums):
    ...
    if(sum(nums) % 2 != 0):
        ...
        return splitArrayHelper(0, nums, sum(nums) // 2)
...
def splitArrayHelper(start, nums, target):
    ...
    if(start >= len(nums)):
        ...
        return splitArrayHelper(start + 1, nums, target - nums[start]) or \
               splitArrayHelper(start + 1, nums, target)
...
def splitOdd10(nums):
    ...
    if(sum(nums) % 2 == 0):
        ...
        return splitOdd10Helper(0, nums, sum(nums) // 10, sum(nums))

  
def splitOdd10Helper(start, nums, target1, target2):

    if(start >= len(nums)):

        return (target1 == 0 and target2 % 2 != 0)

    elif(nums[start] % 5 == 0 and nums[start] % 2 != 0):

        return splitOdd10Helper(start + 1, nums, target1 - 1, target2) or \
               splitOdd10Helper(start + 1, nums, target1, target2 - nums[start])

    elif(nums[start] % 5 == 0):

        return splitOdd10Helper(start + 1, nums, target1 - 1, target2) or \
               splitOdd10Helper(start + 1, nums, target1, target2)
    
    elif(nums[start] % 2 != 0):

        return splitOdd10Helper(start + 1, nums, target1, target2 - nums[start]) or \
               splitOdd10Helper(start + 1, nums, target1, target2)

    else:

        return splitOdd10Helper(start + 1, nums, target1 - 1, target2) or \
               splitOdd10Helper(start + 1, nums, target1, target2 - nums[start]) or \
               splitOdd10Helper(start + 1, nums, target1, target2)
...
def split53(nums):
    ...
    if (sum(nums) % 2 != 0):
        ...
        return split53Helper(0, nums, sum(nums), sum(nums))

def split53Helper(start, nums, target1, target2):
    ...
    if (start >= len(nums)):
        ...
        return split53Helper(start + 1, nums, target1, target2 - nums[start]) or \
               split53Helper(start + 1, nums, target1, target2)

    elif(nums[start] % 5 == 0):

        return split53Helper(start + 1, nums, target1 - nums[start], target2) or \
               split53Helper(start + 1, nums, target1, target2)

    else:

        return split53Helper(start + 1, nums, target1 - nums[start], target2) or \
               split53Helper(start + 1, nums, target1, target2 - nums[start])
